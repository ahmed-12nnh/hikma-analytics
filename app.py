...
# ---------------------------------------------------------
# 🎨 الشريط الجانبي المخصص (مثل Gemini) - تم إصلاح الزر
# ---------------------------------------------------------
...
"""
    ...
    # الشريط الجانبي المخصص بالكامل مع إصلاح زر القائمة
    # تم إضافة id="sidebarToggleBtn" للزر وتحديث الجافا سكربت للعمل بشكل مباشر
    sidebar_html = f"""
...
<script>
    // استخدام دالة فورية لضمان عزل النطاق وتنفيذ الكود فوراً
    (function() {{
        ...
        function toggleMenu(e) {{
            // منع انتشار الحدث لتجنب التعارض
        ...
        // ربط الحدث مباشرة بالعنصر (أكثر ثباتاً من onclick في HTML)
        if (toggleBtn) {{
        ...
        document.addEventListener('click', function(e) {{
            if (sidebar && sidebar.classList.contains('expanded') && !sidebar.contains(e.target)) {{
                // التأكد من أن النقر ليس على زر الفتح نفسه
                if (!toggleBtn.contains(e.target) && !historyBtn.contains(e.target)) {{
