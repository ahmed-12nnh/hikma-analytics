...
# ---------------------------------------------------------
# 🎨 إعدادات الصفحة والتصميم الخارجي (الثيم الكحلي والذهبي)
# ---------------------------------------------------------
st.set_page_config(page_title="منصة التحليل الاستراتيجي", page_icon="🦅", layout="wide")

# CSS الواجهة (نفس التصميم الذي تحبه)
st.markdown("""
<style>
...
# ---------------------------------------------------------
# 🎨 القوالب (CSS Styles for Reports)
# ---------------------------------------------------------

# 1. القالب الرسمي (للكتب والمخاطبات)
STYLE_OFFICIAL = """
<style>
...
# 3. القالب التحليلي (إحصائيات وهرمي)
STYLE_ANALYTICAL = """
<style>
...
# 4. 🔥 القالب الجديد: العرض التقديمي التفاعلي (Presentation Mode)
# تم نقل هذا الكود حرفياً من الملف الذي أرسلته لضمان نفس التصميم
STYLE_PRESENTATION = """
<style>
    :root {
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h3 { color: var(--primary-blue); font-size: 1.6rem; border-bottom: 1px dashed #ccc; padding-bottom: 5px; }
    p, li { font-size: 1.2rem; line-height: 1.8; text-align: justify; }
    ...
    .main-title { font-size: 3.5rem; color: var(--white); text-shadow: 0 4px 10px rgba(0,0,0,0.5); }
    .sub-title { font-size: 1.8rem; color: var(--gold-main); font-weight: 300; }
...
# 5. 🆕 القالب الجديد: الملخص التنفيذي الحديث (Modern Executive)
STYLE_EXECUTIVE = """
<style>
...
# JavaScript لتشغيل العرض التقديمي
SCRIPT_PRESENTATION = """
<script>
    ...
    function nextSlide() { if (currentSlideIndex < totalSlides) { currentSlideIndex++; updateSlide(); } }
...
# أزرار الاختيار (تمت إضافة الأنماط الجديدة)
...
if st.button("🚀 بدء المعالجة وإنشاء التقرير الكامل"):
    ...
    if not full_text.strip():
        ...
        try:
            ...
            # التوقيع الموحد (يتم إضافته لجميع التقارير ما عدا العرض التقديمي له هيكل خاص)
            unified_signature = """
            ...
            if "الرسمي" in report_type:
                ...
                design_rules = "Style: Official Corporate Report. Use <div class='card'>, HTML <table>, <ul> lists."
            
            elif "الرقمي" in report_type:
                target_css = STYLE_DIGITAL
                file_label = "Digital_Dashboard"
                design_rules = "Style: Modern Digital Dashboard. Use <section id='summary'>, <article class='card'>, <div class='goal'>."
            
            elif "التحليل" in report_type:
                target_css = STYLE_ANALYTICAL
                file_label = "Deep_Analysis"
                design_rules = "Style: Statistical Hierarchy. Use <div class='stats-grid'>, <div class='pyramid-grid'>, <div class='bar-container'>."
            
            elif "ملخص" in report_type:
                target_css = STYLE_EXECUTIVE
                file_label = "Executive_Summary"
                design_rules = "Style: Modern Executive Summary. Use <div class='executive-summary'>, <div class='grid-2'> with <div class='metric-box'>."
            ...
            # 3. هندسة الأمر (Prompt) - تم إزالة التاريخ الإجباري
            prompt = f"""
            ...
            **CRITICAL INSTRUCTIONS:**
            ...
            2. **DATE:** Do NOT force a specific year. Use the dates found in the input text. If no date is found, leave it generic.
            ...
            **LANGUAGE:** Arabic (Professional).
            ...
            st.success("✅ تم الإنشاء بنجاح (بدون اختصار)!")
