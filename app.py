    ...
    try:
        if uploaded_file.type == "application/pdf":
            try:
                ...
                for page in doc:
                    # 🔧 تعديل استراتيجي: إضافة sort=True لقراءة النصوص العربية بشكل مرتب بصرياً
...
def process_report(user_text, uploaded_file, report_type):
    ...
    try:
        ...
        # --------------------------------------------------------------------------------
        # ⚡ تعديل استراتيجي: تعليمات صارمة (Zero-Tolerance for Hallucinations)
        # --------------------------------------------------------------------------------
        prompt = f"""
أنت آلة نسخ وتحويل دقيقة جداً (Strict Verbatim Converter).
المهمة: تحويل النص المدخل إلى HTML باستخدام التصميم المطلوب، ولكن **دون تغيير حرف واحد** من المحتوى الأصلي.

🚫 المحظورات (Strictly Forbidden):
1. **ممنوع** التلخيص أو الاختصار.
2. **ممنوع** تأليف نصوص غير موجودة (No Hallucinations).
3. **ممنوع** تصحيح الأخطاء الإملائية أو النحوية (انسخ الخطأ كما هو إذا وجد).
4. **ممنوع** تغيير ترتيب الأسماء أو الأرقام.

✅ المطلوب (Instructions):
1. انسخ المحتوى النصي **كما هو تماماً** (Word-for-Word) وضعه داخل هيكل HTML.
2. إذا كان هناك جداول في النص، حولها إلى `<table class="data-table">` بنفس البيانات بالضبط.
3. استخدم الكلاسات التالية فقط للتصميم:
{design_rules}

المخرج النهائي يجب أن يكون كود HTML فقط (داخل Body) يحتوي على **كامل** النص الأصلي.
...
"""
        ...
        progress_messages = [
            "🔍 تحليل النص الأصلي...",
            "⚙️ ضبط وضع النسخ الحرفي...",
            "📊 هيكلة البيانات (بدون تعديل)...",
            "🎨 تطبيق القالب التصميمي...",
            "✍️ إنشاء جداول البيانات...",
